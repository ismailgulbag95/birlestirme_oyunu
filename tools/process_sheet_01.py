...
SHEET_PATH = r"C:\Users\ismai\.gemini\antigravity-ide\brain\f1f0f847-c955-4abf-af01-da0772fab632\sheet_01_elements_nature_1789712335719.jpg"
OUTPUT_DIR = r"d:\github\birlestirme_oyunu\public\textures\items"
RAW_DIR = r"d:\github\birlestirme_oyunu\assets\raw_sheets"

os.makedirs(OUTPUT_DIR, exist_ok=True)
os.makedirs(RAW_DIR, exist_ok=True)

# 3 satır x 5 sütun = 15 item
ITEMS_GRID = [
    ["basinc", "oksijen", "atom", "petrol", "karbon"],
    ["karbondioksit", "algae", "plankton", "arctic", "beach"],
    ["bubble", "boiling", "branch", "anchor", "aquarium"]
]

def remove_background_pure_pil(img_rgb, tolerance=32):
    """
    Saf NumPy + BFS Floodfill ile dış kenarlardan başlayarak stüdyo arka planını siler.
    İç beyazlıklar ve saydam parlamalar korunur.
    """
    w, h = img_rgb.size
    rgb_arr = np.array(img_rgb.convert("RGB")).astype(np.float32)
    
    # Köşelerden arka plan rengini tahmin et
    corner_pixels = [
    ...
    # Dış kenarlardan BFS flood-fill
    visited = np.zeros((h, w), dtype=bool)
    queue = deque()
    
    # 4 kenarı kuyruğa ekle
    for x in range(w):
    ...
    # visited True olanlar dış arka plandır -> Alfa 0, diğerleri 255
...
def process_sheet():
    ...
    print(f"Sheet boyutu: {sheet_w}x{sheet_h}")
    ...
    print(f"\nToplam {saved_count} item görseli başarıyla üretildi ve kaydedildi!")
